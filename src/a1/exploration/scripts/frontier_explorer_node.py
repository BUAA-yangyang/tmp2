...
class FrontierExplorer:
    ...
    def __init__(self):
        ...
        # Set whenever build_roi_mask has to clip the ROI to the grid's
        # sensor margin. Frontier extraction is masked by that ROI, so a
        # clipped run can reach "no frontier remains" with part of the
        # floor excluded; the result must say so rather than only warn.
        self.roi_clipped = False
        ...
        # A room left on budget rather than on proof gets this many further
        # transactions before it is allowed to count toward the floor quota.
        self.room_revisit_attempts = int(
            self.param("frontier/room_priority/revisit_attempts", 2)
        )
    ...
    def complete_room_branch(self, branch, proven=True):
        """Record a covered room and report whether the floor quota is met.

        ``proven`` is explore_room_transaction's own verdict: True only when
        no reachable frontier remained inside the room. It used to be dropped
        on the floor here -- the transaction set last_room_transaction_proven
        and nobody read it -- so a room the transaction had explicitly given up
        on ("could not bound the room; leaving") still counted toward 4/4.
        Measured on mf13: three rooms proven, one abandoned, "completed 4/4
        distinct rooms" all the same. An unexplored room is exactly where an
        unseen danger source would be, so it must not close the floor.
        """
        was_new = branch not in self.completed_room_branches
        self.completed_room_branches.add(branch)
        if proven:
            self.unproven_room_branches.pop(branch, None)
        else:
            spent = self.unproven_room_branches.get(branch, 0) + 1
            self.unproven_room_branches[branch] = spent
            rospy.logwarn(
                "room branch %r left on budget, not on proof (attempt %d/%d); "
                "it stays eligible for revisit and does not close the floor",
                branch, spent, self.room_revisit_attempts,
            )
        floor_complete, revivable = room_completion_state(
        ...
        rospy.loginfo(
            ...
            "station=%d side=%s new=%s proven=%s pending_revisit=%d",
    ...
    def choose_frontier(self, map_message, robot_pose, frontiers):
        ...
        # Nothing left anywhere. Before the floor is allowed to finish, give a
        # room abandoned on budget one more transaction; its remaining frontier
        # is where an unseen danger source would be.
        _complete, revivable = room_completion_state(
            self.completed_room_branches,
            self.unproven_room_branches,
            self.floor_completion_target,
            self.room_revisit_attempts,
        ...
        if revivable:
            branch = revivable[0]
            self.completed_room_branches.discard(branch)
            self.approached_room_branches.discard(branch)
            rospy.logwarn(
                "reviving room branch %r for one more transaction: it was "
    ...
    def execute(self, goal):
        ...
        try:
                    ...
                    if (
                        ...
                        if (
                            ...
                            if not self.explore_room_transaction(
                                ...
                                self.wait_for_map_update(version)
                                continue
                            floor_complete = self.complete_room_branch(
                                branch, self.last_room_transaction_proven)
            ...
            if self.roi_clipped:
                # Frontier extraction is masked by the ROI, so "no reachable
                # frontier remains" was decided over a region smaller than the
                # one asked for. Say so in the result the mission acts on;
                # a throttled warning in a 70k-line log is not a report.
                result.message = (
                    "DEGRADED (ROI was clipped to the OccupancyGrid sensor "
                    "margin; completion was judged over a reduced region): %s"
                    % result.message
                )
                rospy.logwarn(
                    "floor %d completed with a clipped ROI; the completion "
                    "decision covered less than the requested region",
                    self.floor_id,
                )
