...
class EntrySpeedLimiterTest(unittest.TestCase):
    ...
    def test_restore_failure_keeps_profile_active_for_fail_closed_retry(self):
        ...
        subject = limiter(service)
        subject.apply()
        # restore() now retries transient transport faults, so every attempt
        # must fail for the fail-closed path to be exercised.
        for call in (2, 3, 4):
            service.fail_on_call[call] = "restore transport"
