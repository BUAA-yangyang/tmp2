...
class BuildingBehaviorServer:
    def __init__(self):
        ...
        self.entry_companion_door = rospy.get_param(
            "~entry_companion_door", "elevator_floor_0"
        ).strip()
    ...
    def execute(self, goal):
        ...
        # In the fixed three-floor scene, expose the floor-zero elevator
        # opening at the same time as the public entrance.  This uses only the
        # public SetDoorState interface; localization still comes exclusively
        # from the subsequently observed LiDAR doorway centre.
        companion_message = ""
        if self.entry_companion_door:
            try:
                companion = self.door_service(
                    door_id=self.entry_companion_door, open=True
                )
            except rospy.ServiceException as error:
                self.abort(
                    SpecialBehaviorResult.ERROR_SERVICE_UNAVAILABLE,
                    "opening companion elevator door %s failed: %s"
                    % (self.entry_companion_door, error),
                )
                return
            if (not companion.accepted or
                    companion.state.lower() != "open"):
                self.abort(
                    SpecialBehaviorResult.ERROR_SERVICE_REJECTED,
                    "companion elevator door %s rejected: accepted=%s state=%r message=%s"
                    % (self.entry_companion_door, companion.accepted,
                       companion.state, companion.message),
                )
                return
            companion_message = "; companion elevator door %s is open" % (
                self.entry_companion_door
            )

        result = SpecialBehaviorResult()
        result.success = True
        result.error_code = SpecialBehaviorResult.ERROR_NONE
        result.message = "public door %s is open: %s%s" % (
            door_id, response.message, companion_message
        )
