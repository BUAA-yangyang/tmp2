...
class FloorMappingNode {
 ...
 private:
  ...
  void processCloud(const sensor_msgs::PointCloud2::ConstPtr& message,const geometry_msgs::TransformStamped& to_odom,const geometry_msgs::TransformStamped& to_base) {
    ...
    pcl::PointCloud<pcl::PointXYZI> output_sensor;pcl::transformPointCloud(output_world,output_sensor,sensor_from_odom);sensor_msgs::PointCloud2 output;pcl::toROSMsg(output_sensor,output);output.header.stamp=message->header.stamp;output.header.frame_id=sensor_frame_;output.is_dense=true;cloud_pub_.publish(output);
  ...
  ros::NodeHandle nh_,pnh_;ros::Subscriber cloud_sub_,odom_sub_,localization_sub_,supervisor_sub_;ros::Publisher cloud_pub_,map_pub_,status_pub_,diagnostics_pub_;ros::ServiceServer reset_service_;ros::Timer status_timer_,map_timer_;ros::WallTimer tf_queue_timer_;tf2_ros::Buffer tf_buffer_;tf2_ros::TransformListener tf_listener_;std::mutex mutex_;
