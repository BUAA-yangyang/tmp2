...
class FloorMappingNode {
 ...
 private:
  ...
  void loadParameters() {
    ...
    param("filter/ground_band_below", ground_below_, 0.05); param("filter/ground_clearance", ground_clearance_, 0.08); param("filter/map_ground_clearing_max_height", map_ground_clearing_max_height_, 0.05); param("filter/maximum_obstacle_height", max_obstacle_height_, 1.5); param("filter/voxel_leaf_size", voxel_leaf_, 0.05);
    ...
    param("grid/p_free", p_free_, 0.35); param("grid/p_occupied", p_occ_, 0.80); param("grid/minimum_observations", minimum_observations_, 2); param("grid/occupied_clear_confirmations", occupied_clear_confirmations_, 3);
  ...
  void processCloud(const sensor_msgs::PointCloud2::ConstPtr& message,const geometry_msgs::TransformStamped& to_odom,const geometry_msgs::TransformStamped& to_base) {
    ...
    for(const auto&p:*filtered){const double range=std::hypot(p.x-sx,p.y-sy);if(range<min_range_||range>max_range_||std::hypot(p.x-robot_x_,p.y-robot_y_)<self_radius_)continue;const double h=p.z-ground_->floorZ();const bool is_ground=h>=-ground_below_&&h<ground_clearance_;const bool is_map_clearing_ground=h>=-ground_below_&&h<map_ground_clearing_max_height_;const bool is_obstacle=h>=ground_clearance_&&h<=max_obstacle_height_;if(!is_ground&&!is_obstacle)continue;if(is_ground){clearing_world.push_back(p);++ground_points_;if(is_map_clearing_ground)ground_returns.push_back(&p);}else{obstacle_world.push_back(p);clearing_world.push_back(p);++obstacle_points_;obstacle_returns.push_back(&p);structure_points.push_back(a1_floor_mapping::HeightPoint{{p.x,p.y},h});}}
    grid_->beginFrame();
