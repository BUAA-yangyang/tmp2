...
class DoorWallRecognizer {
 public:
  ...
  DoorWallFrame update(const std::vector<HeightPoint>& obstacle_points, const Point2& sensor_origin) {
    updateWalls(extractSegments(obstacle_points));
 ...
 private:
  ...
  bool validSegment(const WallSegmentGeometry& segment) const {
    return segment.length >= parameters_.minimum_wall_length &&
           segment.residual <= parameters_.line_fit_error &&
           segment.height_support >= parameters_.minimum_wall_height &&
           segment.support_points >= parameters_.minimum_wall_support;
  }

  std::vector<WallSegmentGeometry> extractSegments(const std::vector<HeightPoint>& points) const {
    ...
    while (active.size() >= parameters_.minimum_wall_support &&
      ...
      for (std::size_t iteration = 0; iteration < parameters_.max_ransac_iterations; ++iteration) {
        ...
        if (norm(reduced[second].xy - reduced[first].xy) < parameters_.minimum_wall_length * 0.5) continue;
        ...
        if (inliers.size() >= parameters_.minimum_wall_support &&
            span >= parameters_.minimum_wall_length && score > best_score) {
          best = inliers;
          best_score = score;
        }
      }
      ...
      const auto flush_group = [&]() {
        ...
        if (validSegment(segment) && segments.size() < parameters_.max_segments_per_frame) segments.push_back(segment);
      };
